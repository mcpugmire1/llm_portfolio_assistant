...
| **5P Narrative** | Person, Place, Purpose, Performance, Process | Supplemental context. Process captures methodology; Performance captures metrics and outcomes. Improve retrieval precision and answer quality without replacing the STAR narrative. |

**Design Principle:**
- Hierarchy answers "Where did this happen?"
- Classification answers "What kind of work was this?"
- Use Case(s) + 5P Summary answer "When should this story be retrieved?"
- STAR answers "What evidence proves the capability?"
- 5P answers "What additional context makes the story richer?"

Retrieval is driven primarily by Use Case(s) and 5P Summary; STAR fields provide the factual evidence used to generate grounded interview responses. `public_tags` (generated by `generate_public_tags.py` from Competencies and story content) expose story capabilities as searchable tags for discovery and filtering -- not part of core semantic retrieval.
...
| `Competencies` | `list[str]` | Skills demonstrated; feeds tag generation and skill-evidence matching |
| `Use Case(s)` | `list[str]` | Interview scenarios; strongest retrieval signal, front-loaded in embedding |
| `Interview Questions` | `list[str]` | Explicit question-to-story retrieval signals; included in build_embedding_text at 1200 chars |
| `Project Scope / Complexity` | `str` | Scale, complexity, and contextual weight of the engagement |
