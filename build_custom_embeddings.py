...
def build_embedding_text(story: Dict[str, Any]) -> str:
    """
    Rich embedding optimized for semantic search.
    
    Includes:
    - Theme (high-level capability area)
    - Industry (domain context)
    - Sub-category (specific capability - often includes exact search terms)
    - 5PSummary (core narrative)
    - Tags (semantic keywords generated specifically for search - includes technical specifics)
    
    This gives semantic search enough signal to match:
    - Conceptual queries ("platform engineering")
    - Technical queries ("Kubernetes", "Docker")
    - Synonym variations ("enablement" vs "upskilling")
    """
    
    summary = story.get("5PSummary", "").strip()
    industry = story.get("Industry", "").strip()
    theme = story.get("Theme", "").strip()
    sub_category = story.get("Sub-category", "").strip()
    tags = story.get("public_tags", "")  # Generated by GPT for semantic search
    
    parts = []
    
    # Structured context for relevance
    if theme:
        parts.append(f"[{theme}]")
    if industry:
        parts.append(f"In {industry}:")
    if sub_category:
        parts.append(f"({sub_category})")  # Often contains exact match terms
    
    # Core narrative
    if summary and len(summary) > 50:
        parts.append(summary)
    else:
        # Fallback if 5PSummary is weak
        ...
        fallback = f"Project: {title} at {client}. {situation} {result}".strip()
        if industry:
            parts.append(f"In {industry}: {fallback}")
        else:
            parts.append(fallback)
    
    # Semantic keywords - the whole point of generating tags!
    if tags:
...
if texts:
    logging.info(f"📝 Sample embedding text (first story):")
    logging.info(f"   {texts[0][:500]}...")
    
    # ← ADD THIS: Check the CI/CD story specifically
    cicd_idx = next((i for i, s in enumerate(stories) if 'cloud-native-architecture-cicd' in s.get('id', '')), None)
    if cicd_idx:
        logging.info(f"📝 CI/CD story FULL embedding text:")
        logging.info(f"{texts[cicd_idx]}")  # ← No truncation, show EVERYTHING
        logging.info(f"Length: {len(texts[cicd_idx])} characters")
    ns_idx = next((i for i, s in enumerate(stories) if 'norfolk-southern' in s.get('id', '')), None)
    if ns_idx:
        logging.info(f"📝 Norfolk Southern FULL embedding text:")
