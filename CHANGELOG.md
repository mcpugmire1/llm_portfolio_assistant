...
**MATTGPT-248 -- partial-failure handling.** A partially-failed assessment now renders its assessed rows and an honest count rather than suppressing the summary or asserting a clean match. Per-call exception handling inside `_assess_one_with_index` (try/except per call, not `return_exceptions=True`) means one unassessed requirement does not kill the full assessment. Unassessed rows render with a grey `⋯` badge (`--pill-bg` fill, `--text-secondary` glyph), an icon, and a failure line. `compute_summary_counts` grows from six to eight keys (adds `unassessed` under `required` and `preferred`). Count line reads "Required: N ✓ N ✗ N not assessed" across all three summary surfaces: `_count_spans` on screen, `_ex_count_line` in the export, and `_build_share_text` (which had no summary block before this -- now added). `build_discussion_points` no longer emits the clean-sweep string when unassessed rows are present in results. Log vocabulary: `assess-caught` (exception caught per call), `retrieval-returned-None` (Pinecone miss), `retrieval-raised` (retrieval threw). `retrieve_stories` None-collapse fix: returns `[]` on a Pinecone miss rather than `None`; `retrieval-returned-None` fires before collapse so production traces still distinguish the two.

**MATTGPT-246 -- export and share surface parity (folded in).** Em dash in both generated title strings fixed (colon separator); unit test asserts no em dash in either output. Export HTML gains a legend block. Export gap icon aligned to UI circled-X. UI summary wording aligned to export ("N strong, N gap"). Report gains evidence chips (supporting story listed per non-gap requirement), a summary block, and a plain-text legend footer. Parity rule established and written into the ticket: facts identical across all three surfaces; affordances and actions only where the reader can take them. Applied: "Try again for the full picture" on screen only; export and report use "🐾 I couldn't get to N of these M requirements" (fact, no instruction).

**MATTGPT-247 -- failure-path Sheet writes (folded in).** `log_role_match_assessment` signature extended with two required parameters (no defaults): `unassessed_count: int` and `failure_type: str`. `unassessed_count` is a correctness fix: since -248 shipped, `strong + partial + gap` no longer sums to `required_count + preferred_count` whenever unassessed rows exist. `failure_type` takes one of three explicit values -- `ok`, `gate_rejected`, `retrieval_failed` -- with no empty cells on new rows (empty carries semantic weight for pre-column historical rows). New function `log_role_match_gate_rejection()` writes a distinct event type (`role_match_gate_rejection`) with all count columns explicit `0`. `HEADERS` grows from 33 to 35: `Unassessed Count` at index 33, `Failure Type` at index 34. Prefix test asserts `HEADERS[:33]` equals the historical snapshot -- not a suffix check, which would pass on mid-list insertion (the -086 failure mode). Call sites: gate rejection at `role_match.py:169`, retrieval failure inside `_handle_assessment_error` at `role_match.py:104`, success path extended at `role_match.py:2281`.

Cycle trail: `0c1189f` (Cycle 1 Green, 92/92), `462148c` / `9b0ddc8` (Cycle 1 follow-up Red/Green -- 4 additional tests: entity round-trip, legend position structural, badge fallback invariant, export gap-no-evidence pin), `2937486` / `9bce6f7` (Cycle 2 Red/Green -- producer surface), `e03c228` / `f42e3d7` (Cycle 2 follow-up Red/Green -- notice copy), `1e9be74` / `43ca523` (-247 Red/Green -- query_logger + wiring).
