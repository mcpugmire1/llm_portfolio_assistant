"""
Test Harness for Semantic Router (INTEGRATION)

Validates that the intent classifier correctly accepts/rejects real queries
against real OpenAI embeddings. MATTGPT-216 moved this file from
tests/unit/ to tests/integration/ because every test in it calls the
OpenAI API via is_portfolio_query_semantic() -> _get_embedding(). The
classifier's pure logic (cosine ranking, family lookup, threshold gating)
is unit-tested against fixture vectors in tests/unit/test_semantic_router.py;
this file tests the end-to-end behavior that requires real embeddings to be
meaningful (does OpenAI produce embeddings that make the router pick the
right family for realistic queries).

Run locally with a populated .env:
    pytest tests/integration/test_semantic_router.py -v

Not part of the pre-push CI gate -- requires an OPENAI_API_KEY which the
CI workflow deliberately does not carry.
"""
