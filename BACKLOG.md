...
4. **Low-confidence gate bypass for profile-answerable queries.** "Where did Matt get his degree?" scores `pinecone_score=0.218` against `CONFIDENCE_HIGH=0.25` and is rejected before the prompt is built -- deterministic, four runs. The gate fires before the profile block would help. Add a bypass: if the query matches a profile-answerable shape (education, certification, language, work authorization, location), skip the low-confidence rejection and proceed to prompt assembly with the profile block loaded. **A keyword list against the query is the right mechanism here.** The rejected-approaches block says keyword matching fires on story searches constantly -- true, and that's why it was rejected for routing decisions. The failure mode is different here: a false positive on the gate bypass just means a story query reaches the LLM with the stories it would have had anyway, plus a profile block it ignores. That's exactly the Q6 control result -- no drift. So a loose keyword check is safe for the bypass in a way it wasn't for rendering decisions.

**`matt_profile.json` -- no changes needed.** Verified September 22, 2026: the file already has three education entries including the UGA Romance Languages / French TA entry. The PoC answered the French education question correctly from the education block alone. A separate `languages` field is explicitly out of scope: asserting proficiency ("Matt speaks French") would reintroduce the over-claim the no-inference citation rule exists to prevent, since fluency no longer applies. Education entries cover the corpus-visible record; that is sufficient.
