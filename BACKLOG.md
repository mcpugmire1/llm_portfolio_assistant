...
3. **-240** — Role Match has no rejection contract: no gate before the LLM call, and the failure path stores the error in a variable that is never read. Recruiter-facing gap with no logging. Pair with -089 (same file, same pass).
4. **-089** — Role Match: location, work model, availability. May 22 recruiter finding. Paired with -240 -- both touch Role Match, cheaper to do in one visit than two.
5. **-242** — Streamlit theme defaults leak into widget chrome after `[theme]` block removal Sep 2. `#FF4B4B` focus borders read as validation errors; wrapper backgrounds and widget labels using Streamlit defaults. Fix shape verified live: CSS with `--accent-purple` and app tokens in `global_styles.py`.
...
8. **-144** -- "projects" mislabel at `explore_styles.py:1187,1199`. Adjacent to -146, same file.
9. **-168** — Slot 1 tie or near-tie gets 80% of the synthesis answer. MATTGPT-174 shipped the Top Score distribution August 13; blocker is cleared. Conditional-pin threshold now derivable from accumulated data.
10. **-180** -- Three test files build on a phantom schema and pass against it. Undermines what the unit suite tells us; same class of problem as the gate pointing at the wrong directory.
11. **-128** — Sources panel split by kind, extracted reason lines, trailing question removed. Design settled August 30. Retrieval check and thin-answer shape still open before Code picks it up.
12. **-129 stories 3-5** — Capital One elicitation, Launchpad timeline and downstream impact, Lean Innovation depth. Blocked on elicitation.

**NEXT**
13. **-235** — Bucket B: resolve LLM-text assertion classes so the pre-push gate can widen. Unblocks -233. Three defects shipped this week through the gap it leaves.
14. **-086** — Environment stamp on every log write. Makes every future log analysis exact rather than heuristic.
15. **-222** — Three operational alarms. Zero-score alarm, extended to distinguish upstream failure (None) from genuine zero-result, would have caught the September 1 outage on the first row.
16. **-223** — Sheet migration, retires local-only borderline and offdomain CSVs.
17. Rest of Role Match: **-160**, -173, -159, -014, -012, -081, -099, -017.
...
**Cross-references:** MATTGPT-202 (`load_star_stories` error handling -- the Streamlit-context concern intentionally kept in `app.py`; defect 1 is that handling being broken). MATTGPT-224 (st.stop() rejection blanking -- different cause, same symptom on screen; shipped `92370b3`).
...
**Scope:** Two classes of audit. Both output a list of findings with file and line; no fix within this ticket -- each finding is evaluated separately for risk.

**Class 1 — Greppable dead/misnamed code:**
...
**Class 2 — Rules applied in one path, missing in another (August 28, 2026):**
A rule implemented in one code path and absent in another the rule arguably covers. Not greppable -- finding these requires reading both paths. Code is the right tool: for each special case or exclusion rule, report every site where it is applied and every site where it arguably should be and is not. Report only; Matt decides which gaps are deliberate.

Four rules to start with:
- `Theme == "Professional Narrative"` exclusion (standing rule in ARCHITECTURE.md for Employer-driven derivations)
- `is_generic_client` -- placeholder Client values (`Fortune 500 Clients`, `Independent Project`)
- Title soft-filter from `f1285f1` (Jan 30, 2026) -- pin the story, keep the pool -- applied in `rag_answer`, absent in `get_synthesis_stories`
...
- **Logged:** August 30, 2026
- **Dependencies:** MATTGPT-230 (makes the `None` vs empty-list signal available)

**Alarm 1 -- zero-score:** Fire when `top_score` is exactly 0.000. This is an empty result set, not a weak match. Would have caught the 2026-01-29 index outage on the first occurrence instead of the sixty-first, and the two August recurrences the day they happened. Extend to distinguish Pinecone upstream failure (`None` return from `pinecone_semantic_search`) from a genuine zero-result query (`[]`): the former is an infrastructure event; the latter is a content gap. Both warrant an alarm, but the alarm message should say which. Reference case: September 1, 2026, 15:47:18 to 15:50:55 -- a 3:37 outage window that this alarm would have caught on the first row. MATTGPT-230 preserves the `None` vs empty-list signal at `rag_service.py:81`; this alarm reads it.
