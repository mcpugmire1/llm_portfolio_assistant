...
df = pd.read_excel(
    "MPugmire - STAR Stories - 6JUN25.xlsx", sheet_name="STAR Stories - Interview Ready"
)
...
missing = filtered[
    filtered["Title"].isnull()
    | filtered["Situation"].isnull()
    | filtered["Task"].isnull()
    | filtered["Action"].isnull()
    | filtered["Result"].isnull()
]
